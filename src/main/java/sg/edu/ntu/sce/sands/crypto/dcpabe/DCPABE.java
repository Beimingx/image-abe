...
public class DCPABE {
    public static GlobalParameters globalSetup(int lambda) {//setup
    ...
    public static Message decrypt(Ciphertext CT, PersonalKeys pks, GlobalParameters GP) {
        List<Integer> toUse = CT.getAccessStructure().getIndexesList(pks.getAttributes());//属性判断

        //if (null == toUse || toUse.isEmpty()) throw IllegalArgumentException;
        if (null == toUse || toUse.isEmpty()) throw new IllegalArgumentException("Not satisfying");
//        if (null == toUse || toUse.isEmpty()){
//            Random random=new Random();
//            byte[] fail=new byte[pks.getUserID().getBytes().length];
//            random.nextBytes(fail);
